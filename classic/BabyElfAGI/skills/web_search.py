...
class WebSearch(Skill):
    ...
    def execute(self, params, dependent_task_outputs, objective):
        ...
        query = self.text_completion_tool(
            f"You are an AI assistant tasked with generating a Google search query based on the following task: {params}. If the task looks like a search query, return the identical search query as your response. {dependent_task}"
            + "\nSearch Query:"
        )
        ...
        print("\033[90m\033[3m" + str(results[:100])[:100] + "...\033[0m")
    ...
    def web_scrape_tool(self, params):
        ...
        info = self.extract_relevant_info(
            params['objective'], text[:11000], params['task']
        )
